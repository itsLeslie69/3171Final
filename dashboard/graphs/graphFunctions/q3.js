...
export default async function getGraph3() {
  ...
  var graph = d3
    ...
    .attr("id", "q3SVG")
    .attr(
      "viewBox", // Making chart responsive
      "0 0 " + width + " " + height
    );
  ...
  d3.csv("../data/customer_satisfaction.csv").then((data) => {
    // x-axis scale
    var xScale = d3.scaleLinear().domain([1, 5]).range([0, innerWidth]);
    ...
    select.onchange = function (e) {
      ...
      function getTitle(index) {
        switch (index) {
          case 1:
            return "Reported Check-In Satisfaction";
          case 2:
            return "Reported Booking Ease Satisfaction";
          case 3:
            return "Reported Satisfaction";
          case 4:
            return "Reported Gate Location Satisfaction";
          case 5:
            return "Reported Baggage Handling Satisfaction";
          default:
            return "Overview";
        }
      }
      ...
      d3.select("#q3SVG")
        .selectAll("#path-" + e.target.selectedIndex)
        .transition()
        .duration(50)
        .style("opacity", 1);
      ...
      d3.select("#q3SVG")
        .selectAll("path" + ":not(#" + "path-" + e.target.selectedIndex + ")")
        .style("opacity", 0.1);

      // Ensuring axes labels do not change opacity
      d3.select("#q3SVG").selectAll("path.domain").style("opacity", 1);
    };
    ...
    for (let i = 1; i < 6; i++) {
      ...
      graph
        ...
        .attr(
          "d",
          createAreaChart // Positioning
            .x((d) => {
              return xScale(parseFloat(d.ranking)) + 0.65;
            }) // Ofsetting to prevent stroke and axis overlap
            .y0(innerHeight - 0.65)
            .y1((d) => {
        ...
        .attr("fill", () => {
          return color[i - 1];
        }) // Offsetting index due to loop starting at 1
        .attr("stroke", "#F5F5DC")
