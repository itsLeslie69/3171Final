...
export default function getGraph2 () {
    ...
    d3.csv('../data/customer_satisfaction.csv').then((data) => {           
        ...
        const svg = d3.select("#pieOne") // Making graph resonsive
        .attr('viewBox',
            "0 0 " + width + " " + height
        )
        ...
        .on('mouseout', function () { // Removing tooltip on mouseout
        ...
        ["Satisfied", "Unsatisifed/Neutral"].forEach((s, v) => {
            const legendRow = legend.append("g")    // Appending each legend icon into a group element
                .attr("transform", `translate(0, ${v * 20})`) // Ofsetting position
                .on('mouseover', function () {
                    d3.select('#pieOne').selectAll('.' + '_' + color[v].slice(1)).transition() // Filling in colours
                ...
                }).on('mouseout', function () { // Fill is used to make the class, which is what the hover targets
                    d3.select('#pieOne').selectAll('.' + '_' + color[v].slice(1)).transition() // Restoring transparency on mouseout
                    .duration(50)
                    .style('opacity', 0.8)
                })
