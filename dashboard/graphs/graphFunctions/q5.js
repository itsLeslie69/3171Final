...
export default function getGraph5() {
  ...
  var graph = d3
    ...
    .attr("id", "q5SVG")
    .attr(
      "viewBox", // Making graph responsive
      "0 0 " + width + " " + height
    );
  ...
  d3.csv("../data/customer_satisfaction.csv").then((d) => {
    ...
    for (let i = 0; i < d.length; i++) {
      for (let z = 0; z < seatClasses.length; z++) {
        if (d[i].Class == seatClasses[z]) {
          // if the csv row's class matches the seat class, push the data to the matching index in groupedData
    ...
    function calculateAvg(arr) {
      // Takes an array, adds all the values, performs avg calculation when loop ends
      let tempTotal = 0;
    ...
    graphGroup
      ...
      .attr("height", (d) => {
        return innerHeight - yScale(parseFloat(d.satisfaction));
      }) // Set height
      .on("mouseover", function (event, d) {
        appendToolTip(
          ...
          d,
          0,
          parseFloat(d.satisfaction).toFixed(2),
          -10,
          -60,
          "Satisfaction"
        );
